...
### [T018] — fate of `ParseCommitsToTrainingStreamAsync`

- **Resolved Answer**: "Option 1"
- **Normalized Requirement**: `ParseCommitsToTrainingStreamAsync` shall be removed from the `IGitCommitParser` interface and from the `GitCommitParser` implementation. The new `IGitCommitParser` interface shall expose only `ParseCommitsToArrayAsync` and `ParseCommitsStreamAsync`. The duplicate training-record assembly logic in `GitCommitParser.ParseCommitsToTrainingStreamAsync` (`src/DiffToJsonLib/GitCommitParser.cs:131-232`), its `BuildTrainingRecord` helper (lines 203-232), and its private `SubstitutePlaceholders` static method (lines 234-243) shall be removed. The new thin-shell `GitCommitParser` (per D002) shall only implement the two retained methods; training-record assembly is the new `TrainingExampleBuilder`'s responsibility, reached via the CLI's explicit composition per D006.
- **Constraints**: The duplicate logic the refactor was meant to eliminate is actually eliminated. The `IGitCommitParser` interface shrinks from three methods to two; the `GitCommitParser` class shrinks accordingly. `ParseCommitsToArrayAsync` is retained as a convenience that materializes the stream. `ParseCommitsStreamAsync` is retained unchanged in shape; the parser-level redaction it currently performs is also removed so the parser produces raw `CommitRecord` instances, with redaction owned by the builder via `RedactionPolicy.Redact(commit, options.Tier)`. The private `SubstitutePlaceholders` static in `GitCommitParser` is replaced by the library-wide `PromptSubstitutor` introduced in T010; the `SubstitutePlaceholders` static in `Program.cs:66-75` is also removed for the same reason. The `IRedactorProvider` constructor dependency of `GitCommitParser` is removed per T009; the new thin-shell constructor signature is `(IProcessInvoker processInvoker)` only. A release note shall be added to `DiffToJsonLib.csproj`'s `<PackageReleaseNotes>` element (matching the existing `DiffToJsonCli.csproj:30-44` pattern) calling out the breaking change for any external NuGet consumer.
- **Cites**: D002, D006, D009

### [T019] — raw-path redaction after the refactor

- **Resolved Answer**: "Option 1"
- **Normalized Requirement**: The CLI's `format == "raw"` branch shall resolve `RedactionPolicy` from DI, wrap `commitParser.ParseCommitsStreamAsync(...)` in a local `async IAsyncEnumerable<CommitRecord>` that calls `policy.Redact(record, redactionTier)` for each record before yielding, and pass the wrapped stream to `diffJsonFileWriter.WriteToJsonFileAsync`. The local iterator shall be a static local function in the action handler (matching the existing `BuildTrainingRecords` pattern at `Program.cs:432-502`), with `[EnumeratorCancellation]` on its `CancellationToken` parameter. The raw writer receives the redacted stream and is unaware of redaction.
- **Constraints**: The policy remains the single source of truth for redaction (per T009); the raw path honors the `--redaction` flag for the first time (previously the parser always redacted the message regardless of the flag, so `--redaction diff` and `--redaction none` were silently ignored on the raw path). The local async iterator is the only place the raw path's redaction is wired; a future contributor refactoring `Program.cs` to extract the raw path into a helper must preserve the wrapper. A regression test shall assert that the raw path's output respects the `--redaction` tier: with `--redaction message`, only the message is redacted; with `--redaction diff`, only the diff is redacted; with `--redaction all`, both are redacted; with `--redaction none`, neither is redacted. A release note shall be added to `DiffToJsonCli.csproj`'s `<PackageReleaseNotes>` element calling out that the raw format's redaction behavior now matches the `--redaction` flag (previously it was effectively "message only" regardless of the flag). The raw path's JSONL output changes shape for any user currently relying on "message is always redacted".
- **Cites**: D007, D010, T009

### [T020] — exact `.csproj` configuration for the exhaustive-switch warning

- **Resolved Answer**: "Option 1"
- **Normalized Requirement**: The library's `DiffToJsonLib.csproj` shall add a `<PropertyGroup>` element containing `<WarningsAsErrors>CS8509</WarningsAsErrors>`. The property shall be added at the top of the file, alongside the existing `<LangVersion>14</LangVersion>` and `<Nullable>enable</Nullable>` elements. The setting applies to the library project only; the test project (`tests/DiffToJsonLib.Tests`, T001) and the CLI project (`src/DiffToJsonCli`) are unaffected unless their own `.csproj` files opt in.
- **Constraints**: T004's compile-time guarantee for the builder's `switch` expression over `AssistantMessageResult` is delivered via CS8509. A future contributor who adds a `switch` statement (not expression) with a missing case is not caught at compile time; CS8524 is not promoted. The test project's `.csproj` may independently enable CS8509 if it grows a `switch` expression over the variant type; this is not required by T020. Other warnings (nullable, IDE suggestions, third-party analyzer warnings) keep their default severity. The configuration is a single property addition; no other `.csproj` change is implied. A regression test for the warning's behavior is unnecessary — the compile failure is the test.
- **Cites**: T004
